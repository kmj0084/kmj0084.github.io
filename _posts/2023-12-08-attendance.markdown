...
```
{r ChatGPT Orange Chart}
# 
# # library(scales)
# 
# # Function to create an orange line chart with a specified date range
# createOrangeLineChart <- function(data) {
# 
#   # Extract the column names
#   x_col <- names(data)[1]
#   y_col <- names(data)[3]
# 
#   # Create the orange line chart with a specified date range
#   ggplot(data, aes_string(x = x_col, y = y_col)) +
#     geom_line(color = "#EB6E1F") +
#     labs(title = data[1,2],
#          x = x_col,
#          y = y_col) +
#     scale_x_date(
...
#       limits = as.Date(c("2021-04-01", "2023-10-31"))
#     ) +
#     theme_minimal()
# }
# your_dataframe <- data.frame(
#   X = seq(as.Date("2021-04-01"), as.Date("2023-10-31"), by = "1 month"),
#   Y = sample(1:100, 30, replace = TRUE)
# )
# 
# createOrangeLineChart(df.fv)
